...
$(document).ready(function() {
  ...
  $('#create-new-channel').on('click', function() {
    ...
    client.getChannelByUniqueName(ROOM_NAME).then(function(channel) {
        console.log(channel, 'poi');
        alert(1);
        $('#overlay').hide();
...
function logIn(identity, displayName) {
  request(`${TWILIO_DOMAIN}/public/api.php?type=chat_token&identity=` + identity + '&channel_name='+encodeURIComponent(ROOM_NAME), function(err, res) {
    if (err) { throw new Error(res.text); }

    var token = res.text;

    userContext.identity = identity;
     console.log('Got new token!', token, userContext);
    ...
    Twilio.Chat.Client.create(token, { logLevel: 'info' })
      .then(function(createdClient) {
        ...
        client.on('tokenAboutToExpire', () => {
          request(`${TWILIO_DOMAIN}/api.php?type=chat_token&identity=` + identity + '&channel_name='+encodeURIComponent(ROOM_NAME), function(err, res) {
            if (err) {
...
function updateMessage(args) {
  ...
  if(message.author == client.user.identity){
    $($el).attr('class', 'myself');
  }else{
    $($el).attr('class', 'other-user');
  }
...
function createMessage(message, $el) {
  var imageDiv = $('<div class="img-box"><img src="https://picsum.photos/200/300" ></div>').appendTo($el);
  var messageDiv = $('<div class="message-class"/>').appendTo($el);
...
function addMessage(message) {
  console.log(message.author, message.body, 'add message');
  console.log('video');
  // message.author == AUTHOR &&
  if(message.body.indexOf('mute-all-video') != -1 ){
    stopAllVideo();
    return;
  }
  if(message.body.indexOf('mute-all-audio') != -1 ){
    muteAllAudio();
    return;
  }
  if(message.body.indexOf('mute-single-audio_') != -1 ){
    let msg = message.body.split("_");
    console.log(msg, 'msg');
    muteSigleAudio(msg[2]);
    return;
  }
  if(message.body.indexOf('mute-single-video_') != -1 ){
    let msg = message.body.split("_");
    console.log(msg, 'msg');
    pauseSigleVideo(msg[2]);
    return;
  }
  var $messages = $('#channel-messages');
...
function setActiveChannel(channel) {
  ...
  $('#muteAllParticipanVideo').on('click', function(e){

    document.getElementById("muteAllParticipanVideo").parentNode.classList.toggle("show-more");
    var body = 'mute-all-audio';
    channel.sendMessage(body).then(function(){});
  });

  $('#stopAllParticipanAudio').on('click', function(e){
   

    document.getElementById("muteAllParticipanVideo").parentNode.classList.toggle("show");
    var body = 'mute-all-video';
    channel.sendMessage(body).then(function(){});
  });

  $(document).on("click",".stopAllParticipanAudioIcon", function(e) {
      this.parentNode.classList.toggle("singleMute");
      var body = 'mute-single-audio_'+$(this).attr('id');
      channel.sendMessage(body).then(function(){});
  });

  $(document).on("click",".stopAllParticipanVideoIcon", function(e) {
      this.parentNode.classList.toggle("singleVideoPause");
      var body = 'mute-single-video_'+$(this).attr('id');
      channel.sendMessage(body).then(function(){});
  });
  ...
  channel.getMessages(10).then(function(page) {
    activeChannelPage = page;
    page.items.forEach(addMessage);
    ...
    if (lastIndex && lastIndex !== newestMessageIndex) {
      console.log(newestMessageIndex, 'asdasd');
...
$(document).ready(function(){
  $(".mobile-chat-icon").click(function(){
    $(".cus-chat-col").toggleClass("slide-chat");
  });
});
