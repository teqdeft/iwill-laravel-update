...
$(function() {
  ...
  $('.plans-guide-block').hide();
   $(".safety-conent-inner .plans-content").on('click',function(e) {
        e.preventDefault();
    ...
    $('#removeLastTeam').on('click', function(e) {
        ...
        if ($('div.custom-safe-field-control').find('input').length > 3)
        {
...
jQuery(function($) {
    ...
    $(".selectPlan").click(function(e) {
        ...
        var getPlanDetail = $('input[name="select_plan"]').data(
            "id"
        );
        var email = $("#email").val();
...
// AWMI select plan
    ...
    $(document).on("submit", "#personl-record-form, #medication-condition-form", function(e) {
        //var card_number = document.getElementById("credit-card-number").getAttribute("aria-invalid");
        if ($(this).valid()) {
            $("#loading").show();
        }
    });
...
$(document).on("click", ".move-next", function(e){
    ...
    $form1 = $("#visitor-detail");
    $form2 = $("#school-details");
...
$(document).ready(function() {
    ...
    if($("#congrats-modal").length) {
         $("#congrats-modal").modal('show');
    }
    if($("#complete-medical-modal").length) {
        $("#complete-medical-modal").modal('show');
    }
    if($("#complete-counseling-modal").length) {
        ...
        if( typeof checkValue == 'undefined' ){
            ...
            $('.errorMedicalCheck').removeClass('displayNone');
                ...
                if( FormType == 'medications' ){
                    ...
                    var content = 'You want to upload document';
            ...
            if( FormType == 'document-manager' ){
                var content = "You don't want to upload document";
            }
