@if(Request::segment(3) == 'step-8')
@php
    $scheduleUrl = url('/schedule-consultation/' . Request::segment(2) . '/step-7/' . Request::segment(4)) . '?action=' . request('action');
    $next_url = url('/schedule-consultation/' . Request::segment(2) . '/step-9/' . Request::segment(4)) . '?action=' . request('action');
@endphp 
                                    ...
                                    <table class="table" style="margin-bottom: 0" border="0">
                                        <thead>
                                            <tr>
                                                ...
                                                <th width="25%">Currently taking?	</th>
										...
										@if($user->user_medications)
											@foreach ($user->user_medications as $medication)
                                            <tr name="Ezetimibe-Simvastatin (oral - tablet) 10 mg-10 mg">
                                                <td>
												{{@$medication->name}}
												<input type="hidden" id="medication-all-tab-deleted-id" value="{{@$medication->id}}">
												</td>
                                                <td>{{ (@$medication->currentlyUse == 'true') ? 'Yes' : 'No' }}</td>
                                                <td>Self Reported</td>
                                                <td>
													@if ($medication->updated_at)
														{{ $medication->updated_at->format('F j, Y') }}
													@endif	
												</td>
                                                <?php /*
                                                <td> 
          <button type="button" onclick="OnClickHealthDocumentDeleted('medication-all-tab','<?php echo $medication->id?>')">
							...
							<div id="medical-allergies-add-section" style="display:none;">
								<form id="health-record-medication-detail-form" method="POST" action="{{ route('store.medication', $user->id) }}">
								@csrf
                ...
                <input type="hidden" name="medicationName" id="medicationName" value="">
									...
									<div class="col-100 form-group">
										...
										<select name="medication-search" id="medication-search-id" class="medication_search-selection">
									...
									<div class="col-100 form-group">
										...
										<input type="number" class="form-control" name="medicationFrequency" id="medicationFrequency" value="" />
									</div>
									...
									<div class="col-100 form-group">
									 ...
									 <div class="col-100 form-group">
										<label>Comment</label>
										<textarea placeholder="Comment*" rows="4" name="medicationComment"></textarea>
									</div>
									
									
									<div class="col-100 cta">
										<button type="button" class="primary-button" onclick="helthRecordMedicationFormSubmit('medication-all-tab')">Save</button>
									</div>
								</form>
							</div>	
                            ...
                            <div class="col-100 cta mt-2">
                                <div class="recorc-cta" style="width: 100%; display: flex; justify-content: space-between; align-items: center;">   
                                    ...
                                    <button class="primary-button" type="submit" onclick="return ScheduleFormSubmit()">Next</button>
                                </div>    
                            </div>
...
function helthRecordMedicationFormSubmit() {
        ...
        const formData = $("#health-record-medication-detail-form").serialize(); // Serialize form data
        ...
        $.ajax({
                ...
                success: function(data) {
                    ...
                    if(data.success) {
                        ...
                        //window.location.href='https://iwilltilimwell.teqdeftdev.com/personal-record?active-tab=tab3';
                    } else {
