            ...
            <div class="tab-container">
                <div class="tab-header account-tab">
                    ...
                    <div class="tab-buttons detail">
                        ...
                        <button class="tab-link {{ request()->get('active-tab') == 'personal-info' || !request()->has('active-tab') ? 'active' : '' }}" data-tab="personal-info">Personal Info</button>
                        @if(in_array($user->planDetailsId, Config::get('constants.family_plan')))
                         @endif
                            <button class="tab-link {{ request()->get('active-tab') == 'dependents-info' ? 'active' : '' }}" data-tab="dependents-info">Dependents</button>
                           
                        <button class="tab-link {{ request()->get('active-tab') == 'pharmacy-info' ? 'active' : '' }}" data-tab="pharmacy-info">Pharmacy</button>
                        <button class="tab-link {{ request()->get('active-tab') == 'password-management' ? 'active' : '' }}" data-tab="password-management">Password Management</button>
                        
                        
                        <button class="tab-link {{ request()->get('active-tab') == 'manage-membership' ? 'active' : '' }}" data-tab="manage-membership">Manage Membership</button>
						
                        <button class="tab-link {{ request()->get('active-tab') == 'transaction-history' ? 'active' : '' }}" data-tab="transaction-history">Transaction History</button>
...
?>
                ...
                <div class="tab-content-detail account-edit-tab">
                        ...
                        @include('mobile.auth.my-account-tab.dependents-info')    
                        @include('mobile.auth.my-account-tab.pharmacy-info')
                        @include('mobile.auth.my-account-tab.password-management')
                        @include('mobile.auth.my-account-tab.manage-membership')
                        @include('mobile.auth.my-account-tab.transaction-history')
